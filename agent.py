...
class AIAgent:
    ...
    def ask_genai(self, query: str, language: str = "en") -> str:
        ...
        system_msg = (
            ...
            "Be accurate, concise (<150 words), and in the requested language. "
            "If user asks outside these schemes, politely say you only support those schemes."
        )
        user_msg = f"Answer in {language}. Citizen query: {query}"
    ...
    def _compose_with_llm(
        ...
        system_msg = (
            ...
            "Always output in the requested language. If not eligible, explain 1–2 reasons and suggest alternatives."
        )
        user_payload = {
            "query": query_en,
            "scheme": scheme,
        ...
        if out_lang != "en":
            try:
                llm_text = GoogleTranslator(source="auto", target=out_lang).translate(llm_text)
                docs = [GoogleTranslator(source="auto", target=out_lang).translate(d) for d in docs]
                steps = [GoogleTranslator(source="auto", target=out_lang).translate(s) for s in steps]
                help_lines = [GoogleTranslator(source="auto", target=out_lang).translate(h) for h in help_lines]
            except Exception:
                pass
